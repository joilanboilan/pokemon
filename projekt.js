...
//_____________________________________________skapning av variabler/objekt osv_________________________________________
...
let searchCountP = document.createElement('p'); //skapar p element
searchCountP.setAttribute("id","searchCount");
main.insertBefore(searchCountP, form);
searchCountP.innerHTML = "Results: ";


let checkBox = document.querySelector('#checkId'); //skapar variabel-referens till checkboxen med id "checkId"
let searchCount = 0; /* deklarerar  search count variabeln utan för funktion så att vi kan komma åt den var vi vill*/
let pokeData = []; /*skapar en array som kommer fyllas med alla nya pokemonObjekt*/
let shinyCheck = false; /* deklarerar shinycheck till false så att den bara blir true senare efter vi klickat i checkboxen*/
let pokeArray = [];
//_____________________________________________________lyssnare nedan________________________________________________________________

document.querySelector('#checkId').addEventListener('change', function(){ /* varje gång checkboxen blir ikryssad eller "oikryssad" så kollar vi dens status med ischecked*/
  isChecked();//tittar om checkboxen är ikryssad eller inte och tilldelar true eller false på shinyCheck
});
    
window.addEventListener('load', function(){
  
  form.addEventListener('submit', function(event){
    event.preventDefault();//förhindrar formulär från att skickas till server
  
     /* tar bort css-klassen som gör elementet osynligt(bootstrap klass) */
    let q = document.querySelector('#search').value;
    search(q, document.querySelector('#content'), pokeArray);
    
    });

    
    for(let j = 1; j < 1011; j++){  //kör igenom fetchen för antal gånger som det finns pokemon
      window.fetch('https://pokeapi.co/api/v2/pokemon/' + j)  //fetchar pokemon efter ID vilket i apisökvägen görs med nummer, därför fungerar det med '+ j'

      .then(function(response) { //efter fetchen före är slutförd körs denna. då får vi en respons som vi sedan returnerar json delen av.
      ...
      .then(function(data) { //sen körs denna där vi skapar kort med datan vi fångar in och skriver ut de på skärmen
        pokeArray[j] = data;
        //console.log(pokeArray[j]);
      });
  
  }});
 document.querySelector('#btnAdd', function(){
...
//_____________________________________________________funtioner nedan_______________________________________________________________

function isChecked(){//kollar om checkboxen är ikryssad och tilldelar shinyCheck true or false
...
function search(query, container, pokeArray) {// TODO: Rewrite to match the specification

   searchCount = 0;/* sätter searchCount till 0 inför varje ny sökning*/
  container.innerHTML = null; /* tar bort allt i container alltså resultat från tidigare sökningar*/
  ...
  for(let i = 0; i<cards.length; i++){
    let cardBtns = [];
    cardBtns[i] = cards[i].getElementsByTagName('button').item(0);
    cardBtns[i].addEventListener('click',function(e){
      let addCard = e.target.parentNode;
      content2.appendChild(addCard);
    })
  }
